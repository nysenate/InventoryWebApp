...
public class DbConnect extends DbManager {
    ...
    public ArrayList getNuSerialList(String nuserialFilter, int numaxResults) {
        ...
        // Quick Fix to allow the Select below to work without error. (9/24/19)
        if (nuserialFilter == null) {
            nuserialFilter = "";
        }
        ...
        try {
            ...
            while (result.next()) {
                ...
                int nuxrefsn = result.getInt("nuxrefsn");
                ...
                invSerialNumber.setNuxrefsn(Integer.toString(nuxrefsn));
    ...
    public List<Location> getLocations(int nuxrefsn) {
        return getLocations(nuxrefsn, true);
    }

    public List<Location> getLocations(int nuxrefsn, boolean closeConnection) {
        ...
        try {
            ...
            if (nuxrefsn > 0) {
                qry = "SELECT MIN(a.dttxnorigin) dttxnorigin, a.cdlocatto, a.cdloctypeto, b.adstreet1, b.adstreet2, b.adcity, b.adstate, b.adzipcode, b.delocat\n" +
                        "FROM fd12issaudit a \n" +
                        "LEFT OUTER JOIN sl16location b ON (a.cdlocatto = b.cdlocat   AND a.cdloctypeto = b.cdloctype AND b.cdstatus = 'A')\n" +
                        "WHERE a.nuxrefsn = ?\n" +
                        "  AND a.cdstatus = 'A'\n" +
                        "GROUP BY a.cdlocatto, a.cdloctypeto,  b.adstreet1, b.adstreet2, b.adcity, b.adstate, b.adzipcode, b.delocat \n" +
                        "ORDER BY 1 DESC;";

                stmt = conn.prepareStatement(qry);

                stmt.setInt(1, nuxrefsn);
                ...
                while (result.next()) {
                    ...
                    location.setCdlocat(result.getString("cdlocat"));
                    location.setCdloctype(result.getString("cdloctype"));
                    location.setAdcity(result.getString("adcity"));
                    location.setAdstate(result.getString("adstate"));
                    location.setAdstreet1(result.getString("adstreet1"));
                    location.setAdzipcode(result.getString("adzipcode"));
                    location.setAdzipcode(result.getString("adzipcode"));
                    location.setDescription(result.getString("delocat"));
                    location.setEntryDate(result.getDate("dttxnorigin"));
...
//            ps.setBlob(2, blobValue);
    ...
    public void removeDeliveryItems(int nuxrpd, String[] items) throws SQLException, ClassNotFoundException, InvalidParameterException {
        ...
        try {
            ...
            for (String item : items) {
                if (!isNumber(item)) {
                    System.out.println("DBConnect removeDeliveryItems NOT A NUMBER: " + nuxrpd + "|" + item);
                    log.warn("Invalid number sent as nuxrpd while removing delivery item: " + item);
                    throw new InvalidParameterException();
                }
                System.out.println("DBConnect removeDeliveryItems: " + nuxrpd + "|" + item);
                ps.setString(1, item);
                ps.setInt(2, nuxrpd);
                ps.addBatch();
            }
    ...
    private boolean isNumber(String item) {
        try {
            if (item != null) {
                Integer.valueOf(item.trim());
            }
            return true;
        } catch (NumberFormatException e) {
            System.out.println("Item is not a number:" + item);
            return false;
        }
    }
